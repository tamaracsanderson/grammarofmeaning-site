...
- **Register every figure on the SSOT — "done" = FINDABLE, not just deployed (S170 lesson, PI-directed).** A figure shipped to `_staging/` is *invisible* until it is a NODE in `engine/data/viz/design_engine.json` with a working `built.file` link AND its `step` value exists in `d.steps` — the design-engine page renders **only** nodes whose `step` matches a declared step, so a new-step figure orphans *silently*. The PI watches the **design-engine gallery**, not loose staging URLs. So the definition of *done* for any figure is: build it → **register the node** (`name` / `one_line` / `step` / `built.file` / `status` / `gate`) **+ add its `step` to `d.steps` if the step is new** → deploy → **confirm it appears in the gallery**. Never report a figure "done / live" on a `_staging` URL alone. (What went wrong: S170 shipped 7 figures to `_staging` but never registered them → the PI couldn't find them → *"it's not good that I could not find it after you said you were done."* The bar is not *deployed* — it is *findable on the SSOT where the PI looks.*)
