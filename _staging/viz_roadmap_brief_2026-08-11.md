...
A **media gallery** that, per passage, surfaces *candidate* images/media the PI picks + places as she builds the reading room. Two source streams (PI): (1) **our library** (the Librarian's media holdings); (2) **the media APIs the Lovable mock used** = **MET Open Access** primarily — the gospel-depths mock carries the API architecture + provenance in `IMAGE-CREDITS.md`, and the **Night Gallery** work is the working precedent (`reading-room-preview/data/gospels_room_config.json` = 34 MET candidate plates, each with credit + crop; open thread in this file). **ELEVATION (PI):** drive the query from **Situate, not just the scripture** — paradigm · location · time period (Mark 16 → 1st-c. Jerusalem · empty-tomb tradition · corpse-care/anointing paradigm → candidates of period tombs / anointing / the place), so media is grounded in the *Sitz*, not a generic illustration. **OWNERSHIP:** Librarian owns media SOURCING (library + API acquisition + the Situate-driven queries — their acquisition lane; they hold the MET architecture); **design-SB builds the gallery-PICKER UI**; provenance §2.4 (CC0/credits) per the Night Gallery pattern. **Flag to the Librarian.** Buildable-shell-now (a picker over the existing gospels_room_config candidates); the Situate-driven query needs the Situate data + a Librarian acquisition query.
