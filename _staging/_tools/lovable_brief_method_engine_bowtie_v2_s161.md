...
| **Situate** | annotate each move in its world (never segments) | Sitz · Paradigm (text-level, stamped on every move) · Frame→Morphospace (text-level) · Witness→Degrees (source-level) · Lineage (move-level → Tributary, off the waist). Note: Sitz/Paradigm are DB-rendered engine-0 outputs (`source_grounding`), ~2 seed texts so far |
